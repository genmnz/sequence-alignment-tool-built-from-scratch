...
def get_sequence(method, label, key):
    if method == "Manual Text":
        ...
        return seq.strip().upper() if seq else ""
        ...
        if uploaded_file is not None:
            try:
                # Read directly from memory (not saved to disk)
                uploaded_file.seek(0)
                text_file = io.TextIOWrapper(uploaded_file, encoding="utf-8")

                record = next(SeqIO.parse(text_file, "fasta"))
                return str(record.seq).upper()

            except StopIteration:
                st.error("No sequence found in the FASTA file.")
                return ""
...
if st.button("Run Alignment"):
    if not seq1 or not seq2:
        st.error("Please enter or upload both sequences!")
    else:
        if "Global" in mode:
